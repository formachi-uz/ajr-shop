        ...
        try:
            ...
            for item in cart_snapshot:
                ...
                if product and product.photo_url:
                    ...
                    await bot.send_photo(target_id, photo=product.photo_url,
...
async def receive_check(message: Message, state: FSMContext, bot: Bot):
    ...
    # Chek guruhiga yuborish
    try:
        if message.photo:
            file_id = message.photo[-1].file_id
            await bot.send_photo(GROUP_CHECKS_ID, photo=file_id,
                                 caption=caption, parse_mode="HTML", reply_markup=kb)
            await bot.send_photo(GLAVNIY_ADMIN_ID, photo=file_id,
                                 caption=caption, parse_mode="HTML", reply_markup=kb)
        elif message.document:
            file_id = message.document.file_id
            await bot.send_document(GROUP_CHECKS_ID, document=file_id,
                                    caption=caption, parse_mode="HTML", reply_markup=kb)
            await bot.send_document(GLAVNIY_ADMIN_ID, document=file_id,
                                    caption=caption, parse_mode="HTML", reply_markup=kb)
        print(f"✅ Chek yuborildi: guruh={GROUP_CHECKS_ID}, admin={GLAVNIY_ADMIN_ID}")
    except Exception as e:
        print(f"❌ Chek yuborishda xato: {e}")
        await message.answer(
            f"⚠️ Chek yuborishda xato yuz berdi.\n"
            f"Iltimos, to'g'ridan @formachi_admin ga yuboring.\n"
            f"Xato: {e}",
            parse_mode="HTML"
        )
