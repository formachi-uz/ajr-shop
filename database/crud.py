...
def _team_type_filter(team_type: str):
    """
    Return a SQLAlchemy filter clause for Product.team_type.

    Backwards-compat:
    - "club"     → team_type == "club" OR team_type IS NULL
                   (legacy mahsulotlar default klub deb hisoblanadi)
    - "national" → team_type == "national" ONLY
                   (NULL hech qachon "national" deb sanalmaydi —
                   shu sabab klub va terma mahsulotlari aralashmaydi)
    """
    if team_type == "national":
        return Product.team_type == "national"
    # "club"
    return (Product.team_type == "club") | (Product.team_type.is_(None))


async def get_team_names_by_type(
    session: AsyncSession,
    category_id: int,
    team_type: str,  # "club" | "national"
) -> list[str]:
    """
    Forma kategoriyasi (1 yoki 2) ichida, berilgan team_type bo'yicha
    UNIQ team_name ro'yxatini qaytaradi (NULL/empty exclude).
    """
    ...
    result = await session.execute(
        ...
        .where(
            ...
            Product.team.isnot(None),
            Product.team != "",
            _team_type_filter(team_type),
        )
        .distinct()
...
async def get_products_by_team(
    session: AsyncSession,
    category_id: int,
    team_type: str,         # "club" | "national"
    ...
    """
    Forma kategoriyasi ichidan team_type/team_name bo'yicha mahsulotlar.
    team_name=None bo'lsa — barcha shu team_type ostidagi mahsulotlar.
    MUHIM: club va national mahsulotlari aralashmaydi.
    """
    if team_type not in ("club", "national"):
        return []
    query = (
    ...
    if team_name:
        query = query.where(Product.team == team_name)
...
async def search_products(
    ...
    name | description | team | brand | model | league bo'yicha.
    """
    ...
    result = await session.execute(
        ...
        .where(
            ...
            (
                ...
                | func.lower(func.coalesce(Product.team, "")).like(pattern)
                | func.lower(func.coalesce(Product.brand, "")).like(pattern)
                | func.lower(func.coalesce(Product.model, "")).like(pattern)
                | func.lower(func.coalesce(Product.league, "")).like(pattern)
            ),
        )
        .order_by(Product.id.desc())
        .limit(limit)
