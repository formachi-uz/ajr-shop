...
async def get_all_categories(session: AsyncSession):
    result = await session.execute(select(Category).where(Category.is_active == True).order_by(Category.sort_order))
...
async def get_products_by_category(session: AsyncSession, category_id: int):
    result = await session.execute(select(Product).where(Product.category_id == category_id, Product.is_active == True))
    return result.scalars().all()
...
async def get_all_products(session: AsyncSession):
    result = await session.execute(select(Product).where(Product.is_active == True).order_by(Product.category_id))
    return result.scalars().all()


async def create_order(session: AsyncSession, user_id: int, payment_type: str, delivery_address: str, comment: str = None):
    order = Order(user_id=user_id, payment_type=PaymentType(payment_type), delivery_address=delivery_address, comment=comment, status=OrderStatus.PENDING)
    session.add(order)
    await session.commit()
...
async def add_order_item(session: AsyncSession, order_id: int, product_id: int, quantity: int, price: float, size: str = None, player_name: str = None):
    item = OrderItem(order_id=order_id, product_id=product_id, quantity=quantity, price_at_order=price, size=size, player_name=player_name)
    session.add(item)
    await session.commit()
    return item
...
async def update_order_status(session: AsyncSession, order_id: int, status: str):
    ...
    await session.execute(update(Order).where(Order.id == order_id).values(status=OrderStatus(status)))

    # Paynet/card checks already decrease stock in check_confirmed.
...
async def get_product_stocks(session: AsyncSession, product_id: int):
    ...
    result = await session.execute(select(ProductStock).where(ProductStock.product_id == product_id).order_by(ProductStock.sort_order))
    return result.scalars().all()


async def set_product_stock(session: AsyncSession, product_id: int, size: str, quantity: int):
    from database.models import ProductStock
    result = await session.execute(select(ProductStock).where(ProductStock.product_id == product_id, ProductStock.size == size))
...
async def decrease_stock(session: AsyncSession, product_id: int, size: str, qty: int = 1) -> bool:
    from database.models import ProductStock
    result = await session.execute(select(ProductStock).where(ProductStock.product_id == product_id, ProductStock.size == size))
    stock = result.scalar_one_or_none()
